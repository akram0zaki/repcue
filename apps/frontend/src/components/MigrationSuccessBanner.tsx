...
const MigrationSuccessBanner: React.FC<MigrationSuccessBannerProps> = ({ onDismiss }) => {
  ...
  return (
    <div 
      ...
      aria-live="polite"
    >
      <div className="bg-green-50 border border-green-200 rounded-lg shadow-lg p-4 mx-auto max-w-lg">
        <div className="flex items-start">
            ...
            <div className="mt-2 text-sm text-green-700">
              ...
              {migrationData.recordsClaimed > 0 && (
                <p className="mt-1 text-xs">
                  {t('auth:migration.success.details')}: {formatRecordTypes(migrationData.tableStats)}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="ml-4 flex-shrink-0">
            <button
              type="button"
              className="bg-green-50 rounded-md p-1.5 text-green-400 hover:text-green-600 focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-offset-2 focus:ring-offset-green-50 transition-colors"
