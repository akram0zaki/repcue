...
- [x] **Hero Section** - Added CSS gradient overlay via `::after` pseudo-element
  - Electric blue to transparent to black gradient (180deg)
  - Dark mode variant with darker blue-to-black
  - Pointer-events: none for proper interactivity
  - Typography updated to `.text-h1` for hero title (from `.text-h3`)
- [x] **Header Section** - Updated typography semantic classes
  - App name: `.text-h2` (from `text-2xl font-bold`)
  - Tagline: `.text-small` (from `text-sm`)
- [x] **Coach Insights** - Component styling preserved (CSS-only changes)
  - Uses existing `.ai-insight-card` and `.ai-badge` classes
  - No JSX modifications needed
- [x] **Upcoming Workout Card** - Modern soft blue surface
  - Gradient background using primary and secondary colors
  - Subtle borders and shadows
  - Hover elevation effect
  - Dark mode variant with darker blue tints
- [x] **Exercise Cards** - Reduced visual weight on buttons
  - Start buttons: outline/ghost style (not filled)
  - Pill-shaped with 9999px border-radius
  - Smaller height (40px) and smaller padding
  - Border color matches primary
  - Hover state with subtle background tint
  - Dark mode: lighter colors for better contrast
- [x] **Section Headers** - Added `.section-title` class
  - Applied to "Popular Exercises" and "Favorite Exercises" sections
  - Uses semantic styling for consistency
- [x] **Language Selector** - Modern minimal styling
  - Added `.language-selector-container` and `.language-select` classes
  - Softer borders and lighter background
  - Proper focus ring styling
  - Hover elevation effect
- [x] **Footer/Legal Section** - Light and minimal
  - Added `.footer-legal-section` and `.footer-legal-link` classes
  - Smaller text size (xs)
  - Muted color with underline on hover
  - Proper focus ring styling for accessibility

**Build Verification** ✅
- TypeScript compilation: 0 errors
- CSS build: All assets compiled successfully
- No warnings or errors in build output
- PWA service worker built successfully

**Files Modified for HomePage Modernization**:
1. `apps/frontend/src/styles/homeParallax.css`
   - Added `.home-hero-wrapper::after` for gradient overlay
   - Dark mode variant for gradient colors
2. `apps/frontend/src/index.css`
   - Added exercise card button styling (outline pills)
   - Added upcoming workout card styling
   - Added section title styling
   - Added language selector and footer styling
3. `apps/frontend/src/pages/HomePage.tsx`
   - Updated hero title from `.text-h3` to `.text-h1`
   - Updated app header from `text-2xl font-bold` to `.text-h2`
   - Updated tagline from `text-sm` to `.text-small`
   - Updated section headers to use `.section-title` class

---

## VISUAL TESTING NEEDED

The following need **visual testing** (all CSS implemented, ready for QA):

1. **HomePage**
   - Hero section renders with proper blue-to-black gradient overlay
   - Hero text stands out clearly with `.text-h1` size
   - AI Coach Insights section displays correctly with updated styling
   - Upcoming workout card shows with soft blue background
   - Popular/Favorite exercises display with lighter outline buttons
   - Exercise "Start" buttons are smaller and less visually dominant
   - Language selector is softer and more minimal
   - Footer text is properly muted and accessible

2. **ExercisePage**
   - Exercise cards render with consistent styling
...
3. **TimerPage**
   ...
   - Start button has gradient and proper styling
...
5. **Dark Mode**
   ...
   - Hero gradient overlay doesn't wash out text
   - Upcoming workout card has proper dark variant
   - Exercise buttons are visible and accessible
   - Language selector and footer are readable
...
### Before
- Hero title: `.text-h3` (smaller, less prominent)
- App header: `text-2xl font-bold` (not semantic)
- Exercise buttons: Large `.btn-primary` (visually dominant)
- Upcoming card: Default surface styling
- Overlay: Tailwind classes (less flexible)

### After
- Hero title: `.text-h1` (larger, more impactful)
- App header: `.text-h2` (semantic, consistent)
- Exercise buttons: Outline pills (lightweight, secondary focus)
- Upcoming card: Soft blue gradient surface
- Overlay: CSS pseudo-element with smooth gradient
- Language/footer: Modern minimal styling
...
All CSS and minimal JSX changes have been implemented following your detailed specifications:

1. **Hero Gradient Overlay** - CSS-only via `::after` pseudo-element
2. **Typography Hierarchy** - Semantic classes for consistent sizing
3. **Visual Weight Reduction** - Exercise buttons changed to outline style
4. **Upcoming Workout Styling** - Soft blue surface with subtle effects
5. **Footer/Language** - Modern minimal appearance with proper focus states

**Quality Metrics**:
- ✅ TypeScript: 0 errors
- ✅ CSS Build: Successful
- ✅ All changes follow token system
- ✅ Accessibility preserved (focus rings, proper colors)
- ✅ Dark mode support complete

Ready for visual testing and final refinements!
